...
vector<FaceInfo> MTCNN::Detect_mtcnn(const cv::Mat& image, const int minSize, const float* threshold, const float factor, const int stage) {
    ...
    if (stage >= 4 && onet_res.size()>0){
        int num = (int)onet_res.size();
        int size = (int)ceil(1.f*num / step_size);
        for (int iter = 0; iter < size; ++iter){
            int start = iter*step_size;
            int end = min(start + step_size, num);
            vector<FaceInfo> input(onet_res.begin() + start, onet_res.begin() + end);
            vector<FaceInfo> res = NextStage(image, input, 48, 48, 3, threshold[2]);
            lnet_res.insert(lnet_res.end(), res.begin(), res.end());
        }
        BBoxRegression(lnet_res);
        onet_res = NMS(lnet_res, 0.4f, 'm');
        BBoxPad(lnet_res, image.cols, image.rows);
    }
//    if (stage >= 5 && lnet_res.size()>0){
//        int num = (int)lnet_res.size();
//        int size = (int)ceil(1.f*num / step_size);
//        for (int iter = 0; iter < size; ++iter){
//            int start = iter*step_size;
//            int end = min(start + step_size, num);
//            vector<FaceInfo> input(lnet_res.begin() + start, lnet_res.begin() + end);
//            vector<FaceInfo> res = NextStage(image, input, 48, 48, 3, threshold[2]);
//            lnet_res.insert(lnet_res.end(), res.begin(), res.end());
//        }
//        BBoxRegression(lnet_res);
//        lnet_res = NMS(lnet_res, 0.4f, 'm');
//        BBoxPad(lnet_res, image.cols, image.rows);
//    }
    if (stage == 1){
        return pnet_res;
...
//
//	for (int n = 0; n < 1;++n){
//		cv::Mat image = cv::imread(prefix+ "office2.jpg");
...
//                cv::circle(image, Point(faceInfo[i].landmark[j],faceInfo[i].landmark[j+1]), 2, Scalar(0, 0, 255), FILLED, LINE_AA);
//            }
//        }
//        cv::imshow("image", image);
//        cv::waitKey(0);
